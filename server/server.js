...
import serviceAccountKey from "./react-js-blog-website-yt-b4a21-firebase-adminsdk-9g3os-60a28b0a28.json" assert {type: "json"}
...
server.post("/signup", (req, res) => {
    ...
    if (fullname.length < 3) {
        return res.status(403).json({ "error": "Full Name must be at least 3 letters" });
    }
    if (!email.length) {
        ...
        for (let i = 0; i < socialLinksArr.length; i++) {
            if (social_links[socialLinksArr[i]].length) {
                ...
                if (!hostname.includes(`${socialLinksArr[i]}.com`) && socialLinksArr[i] != 'website') {
                    return res.status(403).json({ error: `${socialLinksArr[i]} link is invaild. You must enter a full link` })
                }
            }
        }
    } catch (err) {
...
server.post("/create-blog", verifyJWT, (req, res) => {
    ...
    if (isAdmin) {
        ...
        return res.status(500).json({ error: "You don't have permission to create any blog" })
    }
