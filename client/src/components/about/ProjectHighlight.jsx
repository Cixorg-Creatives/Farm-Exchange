    ...
    return (
        <div className='flex flex-col lg:flex-row items-start justify-center gap-4 lg:gap-11 py-6 md:py-10 lg:py-14 old-standard-tt'>
            <div className='flex flex-col items-start justify-start gap-0.5 lg:gap-5 w-full sm:w-[90%] md:w-[80%] lg:w-[495px]'>
                <p className='text-[#859F3E] font-bold text-xl sm:text-2xl md:text-3xl lg:text-[28px] lg:leading-10 uppercase'>Farm Exchange</p>
                <h1 className='text-[#31511E] font-bold text-3xl sm:text-4xl md:text-5xl leading-10 lg:leading-16'>Project Highlights</h1>
            </div>
            <div>
                <div className='relative'>
                    <div className='overflow-hidden w-full sm:w-[450px] md:w-[600px] lg:w-[817px] h-auto sm:h-[400px] md:h-[600px] lg:h-[813px]'>
                        <motion.div key={currentIndex} className='relative' initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 1.0 }} >
                            <img key={currentIndex} src={carouselData[currentIndex].image} alt="" className='w-full sm:w-[450px] md:w-[600px] lg:w-[817px] h-auto sm:h-[400px] md:h-[600px] lg:h-[813px]' />
                            <div className='absolute inset-0 bg-[#073D2C33]'></div>
                        </motion.div>
                    </div>
                    <div className='absolute w-full lg:w-[586px] h-full lg:h-[513px] bg-[#073D2C]/80 lg:bg-[#073D2C] top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2  lg:-translate-x-0 lg:-left-2/3 p-4 sm:p-6 md:p-8 lg:p-6 flex flex-col items-center lg:items-start justify-between'>
                        <motion.div key={currentIndex} className='flex flex-col gap-6' initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 1.0 }}>
                            <p className='text-[#F6FCDF] font-normal text-xl sm:text-2xl md:text-3xl lg:text-3xl leading-8 sm:leading-10 md:leading-11 uppercase'>{carouselData[currentIndex].title}</p>
                            <p className='text-[#D4E0AF] font-normal text-lg sm:text-xl md:text-2xl leading-7 sm:leading-8 md:leading-9'>{carouselData[currentIndex].description}</p>
                        </motion.div>
