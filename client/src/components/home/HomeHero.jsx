      ...
      <div className='hidden xl:flex relative w-full h-full rounded-3xl'>
        ...
        <div className='absolute left-16 top-1/3 bg-transparent flex flex-col'>
          <div className='flex gap-2 md:gap-4 lg:gap-6'>
            <motion.p className='uppercase text-white font-semibold text-[5rem] leading-tight' initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0 }} viewport={{ once: true }}>UNLOCK</motion.p>
            <motion.p className='uppercase text-white font-semibold text-[5rem] leading-tight' initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} viewport={{ once: true }}>LAND VALUE,</motion.p>
          </div>
          <motion.p className='uppercase text-white font-semibold text-[4rem] leading-tight lg:mb-3' initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 1.0 }} viewport={{ once: true }}>DRIVE REAL GROWTH</motion.p>
          <motion.p className='capitalize text-[#D9D9D9] font-semibold text-xs md:text-lg lg:text-2xl leading-tight' initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 1.5 }} viewport={{ once: true }}>Partnering with Landowners to Market, <br /> Sell, and Succeed. Strategic Marketing <br /> Meets Real Estate Expertise </motion.p>
        </div>
          ...
          <div className='h-full flex items-center justify-between pl-5'>
            ...
            <div className='relative'>
              ...
              <div className='absolute inset-0 bg-[#859F3E66] rounded-r-3xl'>
                <p className=' uppercase text-[#073D2C] font-semibold text-base leading-tight py-6 px-4'>We Help You Sell Smarter, Faster.</p>
              </div>
            </div>
          </div>
        </div>
