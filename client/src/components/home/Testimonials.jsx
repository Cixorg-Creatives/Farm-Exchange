        ...
        <AnimatePresence mode='wait'>
          <motion.div key={currentIndex} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className='flex flex-col items-center gap-6 md:gap-12'>
            <div className='h-full flex justify-around lg:justify-between items-center relative'>
              <img src={data[currentIndex].image[0]} alt="" className='w-1/3 md:w-1/6 h-auto aspect-square rounded-md md:rounded-lg lg:rounded-xl' />
              <div className='w-3/4 flex-col items-center justify-end hidden md:flex'>
                <div className='relative'>
                  <p className='capitalize text-[#31511E] font-normal md:text-base lg:text-xl'>{data[currentIndex].testimonial}</p>
                  ...
                  <img src={assets.close_double_quotes} alt="" className='absolute bottom-0 translate-x-1/2 translate-y-3/5 right-0 size-4 md:size-8 lg:size-16' />
                </div>
                <p className='w-full capitalize text-[#31511E] font-normal text-lg lg:text-2xl text-right absolute bottom-0 right-0'>~ {data[currentIndex].author}</p>
              </div>
            ...
            <div className='w-4/5 flex flex-col items-center gap-6 md:hidden '>
              <div className='relative'>
                <p className='capitalize text-[#31511E] font-normal text-xs'>{data[currentIndex].testimonial} </p>
                <img src={assets.open_double_quotes} alt="" className='absolute top-0 -translate-x-[125%] -translate-y-1/2 left-0 size-5 md:size-10' />
                <img src={assets.close_double_quotes} alt="" className='absolute bottom-0 translate-x-0 translate-y-1/2 right-0 size-5 md:size-10' />
              </div>
              <p className='w-full capitalize text-[#31511E] font-normal text-sm text-right'>~ {data[currentIndex].author}</p>
            </div>
          </motion.div>
        </AnimatePresence>
