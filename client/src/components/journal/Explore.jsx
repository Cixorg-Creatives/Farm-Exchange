          ...
          Object.values(data).map((item, index) => (
            <div key={index} className='flex flex-col items-start justify-end'>
              ...
              <div className='flex-col items-start gap-1 lg:gap-2.5 mt-1 md:mt-2 lg:mt-4 text-start w-full h-12 md:h-24 lg:h-36'>
                <p className='capitalize text-[#31511E] font-bold text-xs md:text-lg lg:text-[2rem]'>{item.title}</p>
              ...
              <Link to={item.location} className='group py-1 md:py-2 lg:py-4 px-2 md:px-4 lg:px-6 border-1 border-[#31511E] flex items-center justify-center gap-1 md:gap-1.5 lg:gap-2.5 cursor-pointer'>
                <p className='capitalize text-[#31511E] font-bold text-[10px] md:text-base lg:text-2xl'>{item.button}</p>
