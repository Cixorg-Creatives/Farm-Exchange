    ...
    <div className="py-6 md:py-10 xl:py-14">
      <div className="bg-[#F6FCDF] w-full px-4 md:px-6 lg:px-32 flex flex-col items-center justify-center gap-2 md:gap-4 lg:gap-8">
        ...
        <p className="uppercase text-[#859F3E] font-semibold text-lg md:text-3xl lg:text-[3.5rem] text-center">
          &apos;&apos;Where Every Seed Tells a Story, And Every Field Holds a
          Memory.&apos;&apos;
        </p>
      </div>
      ...
      <div className="relative flex flex-col items-center gap-2 md:gap-3 lg:gap-4 w-full overflow-hidden">
        <img
          src={assets.ellipse1}
          alt=""
          className={`z-2 w-full absolute top-0 -translate-y-1/2 scale-105 transition-all duration-700 ease-in-out ${
          ...
          >
            {extended_images.map((item, index) => (
              <img
                ...
                src={item}
                alt=""
                className={`w-1/4 h-auto aspect-2/3 object-cover px-1 md:px-2 lg:px-4 transition-all duration-700 ease-in-out ${
                  loaded ? "blur-0" : "blur-md"
                }`}
                loading="lazy"
                onLoad={() => setLoaded(true)}
        ...
        <img
          src={assets.ellipse2}
          alt=""
          className={`z-2 w-full absolute bottom-0 translate-y-1/2 scale-105 transition-all duration-700 ease-in-out ${
            loaded ? "blur-0" : "blur-md"
          }`}
          loading="lazy"
          onLoad={() => setLoaded(true)}
