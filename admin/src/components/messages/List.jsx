...
const List = () => {
    ...
    const [seen, setSeen] = useState({});  // Track seen/unseen state per contact
    ...
    return (
        <div className='my-2 md:my-4 xl:my-6 clashdisplay'>
            <div className='grid grid-cols-1 md:grid-cols-[1fr_1fr] lg:grid-cols-[1fr_1fr_1fr] gap-1.5 md:gap-x-3 lg:gap-x-5 gap-y-2 md:gap-y-4 lg:gap-y-6'>
                {contacts.map((item) => (
                    <div key={item._id} className={`relative px-2 md:px-4 lg:px-8 py-2.5 md:py-5 lg:py-9 border border-[#D9E1C3] flex flex-col items-start gap-0.5 md:gap-[3px] lg:gap-1 cursor-pointer rounded-sm md:rounded-md lg:rounded-lg ${seen[item._id] ? 'opacity-60' : ''}`}
                        onClick={(e) => {
                        ...
                        <div className='absolute right-2 md:right-3 lg:right-4 top-2 md:top-4 lg:top-4 flex items-center gap-2 md:gap-3 lg:gap-4'>
                            {
                                seen[item._id] ?
                                    <Button variant='primary' symbol='eye_closed' icon='show' className='delete-button' onClick={() => setSeen(prev => ({ ...prev, [item._id]: !prev[item._id] }))} /> :
                                    <Button variant='primary' symbol='eye' icon='show' className='delete-button' onClick={() => setSeen(prev => ({ ...prev, [item._id]: !prev[item._id] }))} />
                            }
                        ...
                        <p className='capitalize text-[#758A68] font-normal text-xs md:text-sm lg:text-base leading-tight'>
                            {seen[item._id] ? item.message : item.message.slice(0, 30) + "..."}
                        </p>
                    </div>
                ))}
            </div>
                ...
                <div className='fixed inset-0 flex items-center justify-center backdrop-blur-sm p-4' onClick={() => setDeleteConfirm(null)}>
                    <div className='bg-[#F6FCDF] px-4 py-6 border-1 border-[#31511E]/50 max-w-sm w-full flex flex-col items-center rounded-md shadow-lg gap-1 md:gap-2 lg:gap-3' onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className='flex gap-3 mt-4'>
                            <Button loading={deleteLoading} icon='show' symbol='delete' variant='destructive' onClick={() => deleteContact(deleteConfirm._id)} />
                        </div>
                    </div>
                </div>
            )}
            ...
            {selectedItem && (
                <div className="fixed inset-0 flex items-center justify-center backdrop-blur-sm p-4"
                    ...
                    <div className="bg-[#F6FCDF] px-4 py-6 border border-[#31511E]/50 max-w-sm w-full flex flex-col items-start justify-center rounded-md shadow-lg gap-0.5 md:gap-[3px] lg:gap-1"
                        ...
                        <Button icon={'show'} symbol={'close'} variant='destructive' className='mt-1 md:mt-2 lg:mt-3' onClick={() => setSelectedItem(null)} />
                    </div>
                </div>
            )}
        </div>
